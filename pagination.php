...
class Pagination {
    ...
    public $max, $total, $start = 0;
    public $append = '</ul>';
    public $prepend = '<ul class="pagination">';
    public $i = 0;
     ...
     * @param int $current_page set current page 
     */
    function __construct($max, $total, $max_items = 10, $current_page = 0) {
        $this->max = (int) $max;
        $this->total = (int) $total;
        $this->max_items = $max_items;

        # check if the get parameter value is not empty or not higher than the total number of pages.
        $this->get = (!empty($current_page) && $current_page !== 0 && ( $current_page <= $this->pages())) ? $current_page : 1;
    }
    ...
    function get_html($url) {
        $links = $this->prepend . "\n";
        $links .= $this->first('<li><a href="' . $url . '" alt="First Page">First</a> </li>' . "\n", '');
        $links .= $this->previous('<li><a href="' . $url . '" alt="Previous Page">Previous</a> </li>' . "\n", '');
        $links .= $this->numbers('<li><a href="' . $url . '">{nr}</a> </li>' . "\n", '<li><a style="background-color:green;color:white"><strong>{nr}</strong> </a></li>' . "\n");
        $links .= $this->next('<li><a href="' . $url . '" alt="Next Page">Next</a> </li>', '');
        $links .= $this->last('<li><a href="' . $url . '">Last Page</a></li>', '');
        $links .= $this->append . "\n";
        return $links;
    }
